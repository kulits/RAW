...
ROOT_DIR = Path("/is/cluster/fast/pkulits/code/IG-LLM")
...
parser.add_argument("--image-folder", type=str, default="/tmp/images")
...
assert (Path(ROOT_DIR) / args.out_path).parent.exists(), args.out_path
...
if not args.is_2d:
    ...
    if args.annotations_path is not None:
        for image_path in Path("/tmp/images").glob("*"):
        ...
        valid_paths = sorted(Path("/tmp/images").glob("*"))[slice(args.slice_start, args.slice_end, args.slice_step)]
        for image_path in Path("/tmp/images").glob("*"):
